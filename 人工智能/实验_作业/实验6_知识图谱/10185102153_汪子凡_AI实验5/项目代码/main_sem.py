...
def train(**kwargs):
    ...
    best_acc = 0.0
    # model.load('checkpoints/{}_{}.pth'.format(model_name, dataset_name))
    ...
    for epoch in range(opt.num_epochs):
        ...
        for ii, data in enumerate(train_data_loader):
            ...
            model.zero_grad()
            out = model(data[:-1])
            loss = criterion(out, data[-1].type(torch.LongTensor))
            loss.backward()
            optimizer.step()
        ...
        if best_acc < acc:
            ...
            model.save(name=dataset_name)
        # toy_acc, toy_f1, toy_loss = eval(model, train_data_loader, opt.rel_num)

        if epoch % 10 == 9:
            print('{}s'.format(time.time() - start_time))
        print('Epoch {}/{}: train loss: {}; test accuracy: {},  test loss {}'.format(
            epoch, opt.num_epochs, train_avg_loss, acc, eval_avg_loss))
...
def eval(model, test_data_loader, k):
    ...
    for ii, data in enumerate(test_data_loader):
        ...
        if opt.use_gpu:
            ...
            data = list(map(lambda x: x.type(torch.LongTensor), data))
